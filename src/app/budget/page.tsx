...
import { Settings2, Plus, Trash2, Pencil, Check, X, ChevronDown } from 'lucide-react';
...
// ── Category Manager Modal with Emoji Picker ──────────────────
function CategoryManagerModal({ onClose }: { onClose: () => void }) {
  ...
  const [emojiPickerFor, setEmojiPickerFor] = useState<string|null>(null); // catId or 'new'
  ...
  function handleAdd() {
    ...
    addCategory({ contextId:activeContextId, name:newCat.name.trim(), icon:newCat.icon, defaultType:tab, isSystem:false, sortOrder:99, color:undefined });
    setNewCat({ name:'', icon:'📦', type:tab });
    setAdding(false);
  }

  function startEdit(id: string, name: string, icon: string) { setEditingId(id); setEditName(name); setEditIcon(icon); }
  function saveEdit(id: string) {
    if (editName.trim()) updateCategory(id, { name:editName.trim(), icon:editIcon });
    setEditingId(null); setEmojiPickerFor(null);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1 max-h-60 overflow-y-auto">
        {cats.map(cat => (
          <div key={cat.id}>
            <div className={cn('flex items-center gap-3 px-3 py-2.5 rounded-lg group transition-colors', editingId===cat.id?'ring-1 ring-teal-500':'hover:bg-black/5 dark:hover:bg-white/5')}>
              {/* Emoji button — opens picker */}
              <button type="button" onClick={() => setEmojiPickerFor(emojiPickerFor===cat.id?null:cat.id)}
                className={cn('text-lg w-8 h-8 flex items-center justify-center rounded-lg flex-shrink-0 transition-all', emojiPickerFor===cat.id?'ring-2 ring-teal-500 bg-teal-500/10':'hover:bg-black/10 dark:hover:bg-white/10')}>
              ...
              {cat.isSystem ? (
                ...
                <div className="flex gap-1">
                  <button onClick={() => saveEdit(cat.id)} className="p-1 rounded hover:bg-teal-500/20 text-teal-500"><Check size={13}/></button>
                  <button onClick={() => { setEditingId(null); setEmojiPickerFor(null); }} className="p-1 rounded hover:bg-red-500/20 text-red-400"><X size={13}/></button>
                </div>
              ) : (
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => startEdit(cat.id, cat.name, cat.icon)} className="p-1 rounded hover:bg-teal-500/20 text-teal-500"><Pencil size={13}/></button>
                  <button onClick={() => { if(confirm(`Delete "${cat.name}"? Transactions will move to Other.`)) deleteCategory(cat.id); }} className="p-1 rounded hover:bg-red-500/20 text-red-400"><Trash2 size={13}/></button>
                </div>
              )}
            </div>

            {/* Emoji picker for this cat */}
            {emojiPickerFor===cat.id && (
              <div className="ml-3 mt-1 mb-2">
                <EmojiPicker value={editingId===cat.id?editIcon:cat.icon}
                  onChange={(emoji) => {
                    if (editingId===cat.id) { setEditIcon(emoji); }
                    else { updateCategory(cat.id, { icon:emoji }); }
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border" style={{ borderColor:'var(--border)' }}>
            <button type="button" onClick={() => setEmojiPickerFor(emojiPickerFor==='new'?null:'new')}
              className="text-lg w-8 h-8 flex items-center justify-center rounded-lg hover:bg-black/10 dark:hover:bg-white/10 flex-shrink-0 transition-all ring-1" >
              {newCat.icon}
            </button>
            ...
            <button onClick={handleAdd} className="p-1.5 rounded-lg bg-teal-500/20 text-teal-500 hover:bg-teal-500/30"><Check size={14}/></button>
            <button onClick={() => setAdding(false)} className="p-1.5 rounded-lg hover:bg-red-500/20 text-red-400"><X size={14}/></button>
          </div>
          {emojiPickerFor==='new' && (
            <EmojiPicker value={newCat.icon}
              onChange={(emoji) => { setNewCat(f=>({...f,icon:emoji})); setEmojiPickerFor(null); }} />
          )}
        </div>
      ) : (
...
export default function BudgetPage() {
  ...
  const rows = allCats.filter(c => c.defaultType===tab).sort((a,b)=>a.sortOrder-b.sortOrder).map(cat => {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 max-w-[1400px] mx-auto">
      ...
      <div className="rounded-xl p-4 lg:p-5 flex flex-wrap gap-4 lg:gap-8 items-center" style={{ background:'var(--bg-sidebar)' }}>
        {[{ label:'Income',val:formatCurrency(totalIncome),color:'#22C55E'},{ label:'Allocated',val:formatCurrency(totalAllocated),color:'var(--text-primary)'},{ label:'Remaining',val:formatCurrency(Math.abs(remaining)),color:remaining>=0?'#22C55E':'#EF4444'}].map(({ label,val,color }) => (
          <div key={label}><p className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color:'var(--text-muted)' }}>{label}</p><p className="text-xl lg:text-2xl font-bold font-mono" style={{ color }}>{val}</p></div>
        ))}
      ...
      <div className="card overflow-hidden">
          ...
          return (
            <div key={cat.id} className={cn('flex flex-col lg:grid lg:grid-cols-[2fr_80px_1fr_1fr_1fr_2.5fr_100px] gap-2 lg:gap-3 px-4 lg:px-5 py-3 lg:py-3.5 border-b transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.02]', i%2===1&&'bg-black/[0.01] dark:bg-white/[0.01]')} style={{ borderColor:'var(--border)' }}>
              <div className="flex items-center gap-2">
                <button onClick={() => setShowManageCats(true)} className="text-base hover:scale-110 transition-transform flex-shrink-0 w-7 text-center" title="Click to change emoji">{cat.icon}</button>
              ...
              <div className="flex items-center">
                ...
                <input type="number" value={allocated||''} onChange={e => upsertBudgetAllocation({ contextId:activeContextId, month:selectedMonth, categoryId:cat.id, allocated:parseFloat(e.target.value)||0, rollover:0 })} placeholder="0" className="input w-24 text-sm font-mono py-1" />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold w-9 text-right flex-shrink-0" style={{ color:barColor }}>{allocated>0?`${Math.round(pct*100)}%`:'—'}</span>
              </div>
              <div className="hidden lg:flex items-center gap-2">
                <button onClick={() => setShowManageCats(true)} className="p-1.5 rounded hover:bg-teal-500/20 text-teal-500 transition-colors" title="Edit"><Pencil size={12}/></button>
                {!cat.isSystem && <button onClick={() => { if(confirm(`Delete "${cat.name}"?`)) useStore.getState().deleteCategory(cat.id); }} className="p-1.5 rounded hover:bg-red-500/20 text-red-400 transition-colors" title="Delete"><Trash2 size={12}/></button>}
              </div>
            </div>
          );
        })}
        ...
        <div className="px-5 py-3 border-t" style={{ borderColor:'var(--border)', background:'var(--bg-primary)' }}>
          <button onClick={() => setShowManageCats(true)} className="flex items-center gap-2 text-sm font-semibold transition-colors hover:opacity-80" style={{ color:'#0EA5A0' }}>
            <Plus size={14}/> Add New Category
          </button>
        </div>
      </div>
